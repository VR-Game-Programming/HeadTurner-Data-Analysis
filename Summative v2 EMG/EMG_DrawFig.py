...
df = pd.read_csv("Processed Data/Processed T1T2 MCL Value.csv")
RootDir = "."
...
def ReturnProcessData(task):
    ...
    for condition in Conditions:
        ...
        for direction in Directions:
            cur_df = df[df["Task"] == task]
            cur_df = cur_df[cur_df["BedStatus"] == condition]
            num_direction = Directions.index(direction) * 90
            DataDict[condition][direction] = cur_df[str(num_direction)].tolist()
            # print(len(DataDict[condition][direction]))
...
T1_EMG_Dict, T1_EMG_StdDict = ReturnProcessData(1)
...
T1_EMG_Dict, T1_EMG_StdDict = ReturnProcessData(2)
